...
- 在问卷“设置 → 填写体验”中开启。提交后填写者会得到私密跟进链接（同时保存在当前浏览器，首页“我的工单”可找到），可查看处理状态、自己提交的内容，并与管理员来回对话。
- 管理员在答卷详情的“对话”区回复，可同时修改处理状态；填写者回复后答卷标记为“待回复”，列表可按此筛选或按最近活动排序，概览页显示待回复数量。填写者回复已解决或需要补充的工单时，状态会自动回到“待处理”。
- 链接密钥位于 URL `#` 之后，不会出现在服务器日志和来源信息中；服务端只保存哈希。关闭工单模式后历史对话仍可查看，但不再接收回复。填写者回复会触发 Webhook `message.created` 事件。

**通知与邮件（工作台“通知”页，仅所有者）**

- 管理员提醒：新答卷、填写者回复工单时，通过提醒邮箱（SMTP）和 / 或 Telegram 机器人通知；每份问卷可单独关闭。Telegram 可自定义 API 地址以使用反代。
- 发给填写者的邮箱：与提醒邮箱分开配置（含回复地址）。每份问卷可指定“联系邮箱题”（默认自动取第一道邮箱题），并可开启提交回执邮件；开启工单模式时邮件附带跟进链接。
- 答卷详情里回复填写者时可勾选“同时发送邮件”，修改状态时可勾选“邮件通知填写者”；状态变化记录在对话时间线中，填写者在工单页也能看到处理进度。每条消息显示邮件是否送达，失败可重新发送。
- 快捷回复模板供团队回复时一键插入；常用邮箱服务商可一键填入；每个渠道都能发送测试消息。密码与 Token 只写不回显。
- 持久化发件队列：自动通知写入数据库后在后台发送，失败按 30 秒 / 2 分钟 / 10 分钟 / 1 小时重试，服务重启后继续；发送记录显示排队、重试次数与错误原因，失败的消息可手动重试。
- 成员订阅：每位成员在“我的账户 → 我的通知”填写自己的邮箱，选择接收新答卷、工单回复、每日汇总，并可只关注指定问卷；与通知页的固定收件人合并去重。
- 每日汇总：按系统时区在指定时间发送过去 24 小时的新答卷（按问卷）、待处理和待回复数量；可设置无动态时不发送，并可立即发送一次测试。
- 邮件外观：品牌名称、主题色、Logo（https）、回复签名、页脚附加文字，作用于全部邮件，通知页提供新答卷提醒 / 提交回执 / 工单回复三种实时预览。
- 站点地址（用于邮件与 Telegram 中的链接）可在通知页设置。
...
- 个人账户：显示名称、邮箱与通知偏好、修改密码、查看并退出其他登录设备；登录可选“保持 30 天”。
- 两步验证（TOTP）：扫码绑定任意验证器 App，开启时生成 10 个一次性恢复码（可重新生成），验证码防重放；开启后其他设备需重新登录。成员丢失手机时，所有者可在成员页重置其两步验证；`npm run reset-password` 也会一并清除。
- API 令牌：个人令牌可设只读 / 读写权限与 30 天、90 天、1 年或永不过期，只在创建时显示一次，服务端只存哈希，可随时撤销并显示最近使用时间。令牌继承本人角色权限，但成员、系统、通知与账户安全相关接口只接受网页登录。
- Webhook：每份新答卷 POST JSON 到指定地址，可设置 HMAC-SHA256 签名密钥，提供测试按钮和最近投递记录。
- 系统页：存储用量、数据库大小、运行状态、系统时区、带操作人的操作日志（保留最近 2000 条）。
- 自动备份：每日定时生成一致的数据库快照（SQLite 在线备份，不需停机）并增量复制附件，保留最近 N 份；可立即备份、下载任一快照，或下载包含数据库与全部附件的完整归档 ZIP。
- 数据保留期：每份问卷可设置答卷在 30 天至 2 年后自动永久删除（含附件与工单消息），满足数据最小化要求，删除会记入操作日志。
...
server/
  ...
  auth.js          账号、会话、角色权限
  routes/          account · users · forms · responses · public · tickets · notifications · system
  services/        forms · responses · statistics · exports · storage · webhooks · audit · tickets · notify · backup · settings
  lib/             zip.js 流式 ZIP 写入 · totp.js 动态码与恢复码
src/
...
- `uploads/`：附件原文件。
- `backups/`：自动或手动备份生成的 `quesuwa-YYYYMMDD-HHMMSS.sqlite` 快照，以及附件镜像 `backups/uploads/`。

推荐在“系统 → 备份与恢复”开启每日自动备份，并定期下载“完整归档”保存到其他机器；`backups/` 与数据在同一磁盘，只能防误删，不能防磁盘损坏。

恢复：停止服务 → 把要恢复的快照改名为 `data/report.sqlite`（删除旧的 `report.sqlite-wal` / `-shm`）→ 把 `backups/uploads/`（或归档里的 `uploads/`）复制回 `data/uploads/` → 启动，数据库会自动升级到当前版本。也可以直接停机复制整个 `data/`。旧版本（单管理员密码）升级后，原管理密码会成为 `admin` 所有者账号的密码，已有问卷、答卷和状态全部保留。
...
在“我的账户 → API 令牌”创建后，以 Bearer 方式调用 `/api/admin/*`：

```sh
TOKEN=qsw_xxxxxxxx
# 问卷列表
curl -H "Authorization: Bearer $TOKEN" https://survey.example.com/api/admin/forms
# 某问卷的答卷（分页、筛选参数与工作台一致）
curl -H "Authorization: Bearer $TOKEN" "https://survey.example.com/api/admin/forms/<formId>/responses?page=1"
# 导出 CSV
curl -H "Authorization: Bearer $TOKEN" -o responses.csv "https://survey.example.com/api/admin/forms/<formId>/export?format=csv"
# 读写令牌可修改答卷状态
curl -X PATCH -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"status":"resolved"}' https://survey.example.com/api/admin/responses/<responseId>
```

只读令牌只能发 GET 请求；令牌不能访问成员、系统、通知设置、账户与令牌管理接口（返回 403）。脚本请求无需 `Origin` 头；浏览器跨域请求仍会被来源校验拒绝。
...
浏览器测试（需要先构建，并安装 Chrome / Chromium；可用 `CHROME_PATH` 指定路径，找不到浏览器时自动跳过，CI 中强制执行）：
...
它用真实浏览器完成登录、手机端填写提交、在账户页开启两步验证并用新会话走完两步登录，再以手机和桌面两种尺寸打开全部工作台页面，要求无脚本错误且无横向滚动。

接口测试使用临时数据目录，覆盖：登录与来源校验、角色权限、草稿/发布/关闭、访问码、每设备一次、条件分页、各题型答案校验、版本冲突、伪装文件拒绝、附件权限、题目快照、回收站与永久删除、统计（NPS、排序、其他答案）、CSV/JSON/ZIP 导出、会话与改密、重启持久化、反向代理和语言协商、工单与通知（SMTP / Telegram 模拟、发件队列重启续发与手动重试、成员订阅与每日汇总、邮件外观预览）、两步验证（防重放、恢复码一次性、所有者重置）、API 令牌（只读限制、敏感接口拒绝、撤销）、备份（保留份数、下载、防路径穿越、完整归档）与数据保留期。
