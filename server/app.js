...
import { bugTemplate, fieldTypes, choiceTypes, statuses, statusKeys } from '../src/shared.js';

import { t, localeMiddleware, serverLocale } from './i18n.js';
...
function validateForm(body) {
  ...
  const fields = body.fields.map(f => {
    ...
    return { id: f.id, type: f.type, label: textValue(f.label, 200, t('labels.question'), true), description: textValue(f.description ?? '', 1000, t('labels.questionDescription')), required: f.required === true, options };
  });
  if (fields.filter(f => f.type === 'file').length > 2) throw fail(400, 'errors.fileFieldCount');
  return { slug, state: body.state, title: textValue(body.title, 120, t('csv.title'), true), description: textValue(body.description ?? '', 3000, t('labels.description')), thanks: textValue(body.thanks ?? '', 1000, t('labels.thanks')), fields };
}
...
export function createApp({ dataDir, password, production = false, publicOrigin = '', trustProxyHops = 0, seed = true }) {
  ...
  if (seed && db.prepare('SELECT count(*) AS n FROM forms').get().n === 0) {
    ...
    const definition = { ...bugTemplate(serverLocale), slug: 'bug-report', state: 'draft' };
  ...
  app.post('/api/admin/login', limiter(15 * 60_000, 10), (req, res, next) => {
    ...
    if (typeof value !== 'string' || value.length > 512 || !timingSafeEqual(passwordHash, scryptSync(value, salt, 64))) return next(fail(401, 'errors.password'));
  ...
  app.put('/api/admin/forms/:id', (req, res) => {
    ...
    if (!current) throw fail(404, 'errors.formNotFound');
    const def = validateForm(req.body);
  ...
  app.get('/api/forms/:slug', (req, res) => {
    ...
    if (!form || form.state === 'draft') throw fail(404, 'errors.formUnavailable');
    if (form.state === 'closed') throw fail(410, 'errors.formClosed');
    res.json(form);
  });
  ...
  app.post('/api/forms/:slug/responses', limiter(60 * 60_000, 12), (req, res, next) => {
    ...
    for (const file of files) {
      if (!form.fields.some(f => f.id === file.fieldname && f.type === 'file')) throw fail(400, 'errors.fileField');
    }
    for (const f of form.fields) {
      const value = incoming[f.id];
      if (f.type === 'file') {
        const selected = files.filter(x => x.fieldname === f.id);
        if (selected.length > 3 || (f.required && !selected.length)) throw fail(400, 'errors.fileCount', { label: f.label });
        continue;
      }
      if (f.type === 'multi') {
    ...
    if (latest.state !== 'published' || latest.version !== form.version) throw fail(409, 'errors.formChanged');
  ...
  app.use((error, _req, res, _next) => {
    ...
    let status = error.status >= 400 && error.status < 500 ? error.status : 500;
